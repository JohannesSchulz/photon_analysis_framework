...
/pnfs/desy.de/cms/tier2/store/user/jschulz/nTuples/Signal_TChiNg_sorted_V10/
...
fetchScript=getOutputTChi_${version}.sh
...
#	echo $signal_name
  ...
  #    	echo $signal_name $outputFileName $files_to_submit
   ...
   #     echo "hadd -f $outputFolder/${signal_name}.${version}_sel.root $outputFolder/${signal_name}.${version}__*.root" >> $fetchScript
    ...
    echo "rm "TChi"*.sh" >> $fetchScript
    echo "rm ~/"TChi"*.sh.o*" >> $fetchScript
